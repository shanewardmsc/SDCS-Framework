...
# Known services
KNOWN_SERVICES = {"valert", "vpm", "vlog", "mqtt_broker", "vOT-Mgmt", "vIT-Mgmt"}
...
# Create MQTT client
try:
    mqtt_client = mqtt.Client(client_id="vIT_Manager")
...
async def get_machines(request: Request):
    ...
    return templates.TemplateResponse("vIT_Mgmt_Overview.html", {"request": request, "machines": machines_data})
...
def get_container_ip(container_name: str, network_name="docker_compose_IT_NETWORK"):
    try:
