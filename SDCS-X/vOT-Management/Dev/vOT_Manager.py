...
async def home(request: Request):
    try:
        ...
        # Host IP and port extraction for launching
        for container in all_containers:
            # Extract the container ID dynamically
            container_id = container.id  # This gives the container ID (a string)

            # Extract NetworkSettings and IP Address
            network_settings = container.attrs.get('NetworkSettings', {})
            host_ip = network_settings.get('IPAddress', '0.0.0.0')  # Default to 'N/A' if no IP is found

            # Extract Ports (ensure Ports is a dictionary)
            ports = network_settings.get('Ports', {})
            
            # Initialize a list to store port information
            host_ports = []

            # Loop through the ports dictionary
            for port, bindings in ports.items():
                for binding in bindings:
                    # Safely access the protocol, or default to 'tcp' if missing
                    protocol = binding.get('Protocol', 'tcp')  # Default to 'tcp' if no protocol found
                    host_ports.append(f"{port}")

            if not host_ports:
                host_ports = ['N/A']  # If no ports are mapped, return 'N/A'

            if not host_ip:
                host_ip = '0.0.0.0'

            # Output the details
            print(f"Container ID: {container_id} | Host IP: {host_ip} | Host Ports: {host_ports}")
